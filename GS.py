...
def Y_decrease(num):
    ...
    motor.count_d = 0
    move('front')
    global_value.set_value('targetB', 21)
    global_value.set_value('targetD', 21)
    t2 = t1 = time.time()
    while t2 - t1 < 3:
        t2 = time.time()
...
def getPos_6(color): #转盘打靶识别物料位置
    ...
    rotated_temp = rotated_temp[140:340]
...
def arm_cpq_2():
    ...
    while 1:
        ...
        if MoveOrStatic_2() == 'static':
            x, y, k= getPos_9(ys[3])
            if get_order_cpq()[ys[3]] == 2 and k < 4000000:
                break
    cv2.imwrite('t4.jpg', global_value.get_value('frame_up'))
    arm_recover()
    time.sleep(1.2)
    ...
    while 1:
        ...
        if MoveOrStatic_2() == 'static':
            x, y, k= getPos_9(ys[4])
            if get_order_cpq()[ys[4]] == 2 and k < 4000000:
                break
    cv2.imwrite('t5.jpg', global_value.get_value('frame_up'))
    arm_recover()
    time.sleep(1.2)
    ...
    while 1:
        ...
        if MoveOrStatic_2() == 'static':
            x, y, k= getPos_9(ys[5])
            if get_order_cpq()[ys[5]] == 2 and k < 4000000:
                break
    cv2.imwrite('t6.jpg', global_value.get_value('frame_up'))
    arm_recover()
    time.sleep(1.5)
    arm_initialize()
################################################################
